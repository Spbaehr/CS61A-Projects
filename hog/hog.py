...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score == 0:
        return 1
    mod = opponent_score % 10
    func = digit_fn(mod)
    change = func(player_score)
    opponent_score //= 10
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if(score0>score1) and last_leader:
        return 0, f"Player 0 takes the lead by {score0-score1}"
    elif(score1>score0) and not last_leader:
        return 1, f"Player 1 takes the lead by {score1-score0}"
    elif(score0==score1):
        return None, None
    elif(score1>score0):
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    optimal_result = 0
    while i <= 10:
        curr_result =  make_averaged(roll_dice, total_samples)(i,dice)
        if curr_result > optimal_result:
            optimal_result = curr_result
            dice_to_roll = i
        i += 1
    return dice_to_roll
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    hefty_result = hefty_hogs(score,opponent_score)
    potential_score = score + hefty_result
    if ((potential_score%10) == (opponent_score%10)) and hefty_result:
        return 0
    return hefty_hogs_strategy(score, opponent_score,threshold,num_rolls)  # Remove this line once implemented.
    # END PROBLEM 11
