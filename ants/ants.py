    ...
    def add_to(self, place):
        ...
        if curr_ant is None:
            ...
            if place.ant.is_container and not self.is_container:
                ...
                val = place.ant is Ant
                print(val)
                print(place.ant.is_container())
    ...
    def nearest_bee(self):
        ...
        while True:
            if curr_place.is_hive:
                ...
                #print(random_bee(bees).place - self.place)
                if self.min_range <= beeLoc - antLoc <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        Beeees = self.place.bees[:]
        death_damage = self.damage + amount
        if health <= 0:
            super().reduce_health(amount)    
            for bee in Beeees:
                Bee.reduce_health(bee,death_damage)
        else: 
            super().reduce_health(amount)
            for bee in Beeees:
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if self.chewing == False and len(self.place.bees):
            bee = random_bee(self.place.bees)
            Bee.reduce_health(bee,bee.health)
            self.chew_timer = self.time_to_chew
            self.chewing = True
        elif self.chewing and self.chew_timer > 0:
            self.chew_timer -=1
        elif self.chewing == True and self.chew_timer <= 0:
            self.chew_timer = self.time_to_chew
            self.chewing = False
            if self.chewing == False and len(self.place.bees):
                bee = random_bee(self.place.bees)
...
class ContainerAnt(Ant):
    """
    ontainerAnt can share a space with other ants by containing them.
    """
    is_container = True
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        if not self.ant_contained and other is not ContainerAnt:
            return True
        return False
